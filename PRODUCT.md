...
Single user: Jordan Smith, an entry-level SOC/GRC/Network Security job-seeker in Wellington, NZ. This is a personal tool for his own use only, run locally on his own PC — not multi-tenant, no other accounts, no plans to share or publish a version for other job-seekers.

## Product Purpose

A local job-application tracker ("Job Search HQ") that replaces spreadsheets/Notion-only tracking with a single source of truth for Jordan's active pipeline: logging roles, tracking status through researching → applied → interview → offer, and triggering tailored CV/cover-letter drafting for a given role without leaving the tracker. Success means fewer stale, forgotten applications and less manual copy-pasting between job ads, his CV, and Notion.
...
- Data lives in a single JSON file (`App/data/applications.json`) on Jordan's PC, written atomically by the Express backend.
...
- Desktop toast notifications (via `node-notifier`) nudge Jordan when an "applied" entry has gone quiet past a stale threshold, so follow-ups aren't missed.
...
- Single-password login gate (timing-safe check, signed session cookie) since the app is reachable over Tailscale from Jordan's phone, not just localhost — see `App/DESIGN.md` for the auth model. Still effectively single-user: no accounts, no per-user data.
...
- CV/cover-letter content must be grounded only in Jordan's real base resume PDF and a living `Candidate Key Facts.md` file — never invented. Standing content rules currently include: use `jordan.smith.demo@example.com` as contact email, always spell out "Riverside Institute of Technology, Wellington" in full, and produce both `.docx` and `.pdf` for every drafted CV (the cover letter is `.docx`-only, since 20 Aug 2026 — not a defect).
...
2. **Automate the tedious, keep judgment calls with Jordan.** CV drafting, staleness checks, and Notion sync are automated; content decisions (what to disclose about visa status, whether a role is a fit) stay his call, surfaced clearly rather than decided silently.
...
4. **Personal tool, not a product to please a market.** Design and feature decisions optimize for Jordan's own workflow and taste, not for generalizing to unknown future users.
